...
const BACKEND_URL = 'https://rajatdhoot.com/api'; // Update with your backend URL
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  switch (request.action) {
      ...
      (async () => {
        try {
          ...
          if (modelConfig.type === 'gemini') {
            const response = await fetch(`${BACKEND_URL}/gemenai`, {
              method: 'POST',
              headers: {
            ...
            const response = await fetch(`${modelConfig.host}/api/chat`, {
              method: 'POST',
            ...
            const data = await response.json();
            sendResponse(data.message.content);
          }
        } catch (error) {
          console.error(error);
    ...
    case 'INIT_OLLAMA':
      currentConfig = {
        type: 'ollama',
        host: request.payload.ollama_host,
        model: request.payload.ai_model,
      };
      return true;
    case 'INIT_OPENAI':
      ...
      return true;
    case 'INIT_CLAUDE':
      initClaude({ ai_key: request.payload.claude_key, model: request.payload.claude_model });
      return true;
